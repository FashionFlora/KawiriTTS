...
def load_checkpoint(checkpoint_path, models, optimizer=None):
    ...
    if isinstance(models, list):
        for i, model in enumerate(models):
            saved_state_dict = checkpoint_dict['models'][f'model_{i}']
        ...
        for k, v in state_dict.items():
            try:
                ...
                new_state_dict[k] = v
        if hasattr(models, "module"):
            models.module.load_state_dict(new_state_dict)
        else:
    ...
    return models, optimizer, learning_rate, iteration



def save_checkpoint(models, optimizer, learning_rate, iteration, checkpoint_path):
...
def load_wav_to_torch(full_path):
    data, sampling_rate = sf.read(full_path, always_2d=False)
